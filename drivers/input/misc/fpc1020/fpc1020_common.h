...
#define FPC1020_INPUT_NAME                      "fpc_input"
#define FPC1020_TOUCH_PAD_DEV_NAME              "fpc_dpad"
...
typedef enum {
	INPUT_STATUS_NONE,
	INPUT_STATUS_WAIT_FOR_WAKEUP,
	INPUT_STATUS_WAIT_FOR_LONGKEY,
} fpc1020_input_status_t;
...
typedef struct fpc1020_key_event {
	unsigned int key;
	int dir;
} fpc1020_key_event_t;
...
typedef struct {
	...
	u32                    reset_gpio;
	u32                    irq_gpio;
	int                    single_click_key_code;
	int                    double_click_key_code;
	int                    long_key_code;
	int                    long_key_detect_time;
	int                    change_up_down_layout;
	int                    irq;
	...
	int                    dynamic_debug;
	int                    finger_is_up;
	struct hrtimer         finger_up_timer;
	struct timespec        ts_finger_up;
} fpc1020_data_t;
...
#define fpc1020_dynamic_debug(fmt, args...) \
	if (fpc1020->dynamic_debug) \
		dev_err(&fpc1020->spi->dev, fmt, ##args);
