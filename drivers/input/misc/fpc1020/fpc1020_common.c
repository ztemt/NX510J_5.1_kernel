...
static const struct chip_struct chip_data[] = {
	...
	{FPC1020_CHIP_1140B, 0x140b, 1, FPC1140_ROWS, FPC1140_COLUMNS, FPC102X_ADC_GROUP_SIZE, 5000},
...
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
	...
	while (counter) {
		...
		gpio_direction_output(fpc1020->reset_gpio, 0);
		...
		if (!error) {
			pr_debug(KERN_INFO "%s OK !\n", __func__);
			counter = 0;
		} else {
			pr_warn("%s timed out,retrying ...\n",
				__func__);
...
int fpc1020_wait_for_irq(fpc1020_data_t *fpc1020, int timeout)
{
	...
	if (!timeout) {
		result = wait_event_freezable(
				fpc1020->wq_irq_return,
				fpc1020->interrupt_done);
	} else {
		result = wait_event_freezable_timeout(
				fpc1020->wq_irq_return,
	...
	if (result < 0) {
		dev_err(&fpc1020->spi->dev,
			 "wait_event_freezable interrupted by signal.\n");
...
#ifdef MANUAL_CS
	if (gpio_is_valid(fpc1020->cs_gpio))
		gpio_direction_output(fpc1020->cs_gpio, 1);
#endif
/*
	dev_dbg(&fpc1020->spi->dev,
			...
			u8 wait_irq_mask)
{
	...
	if ((error >= 0) && wait_irq_mask) {
		...
		if (error >= 0)
			...
			dev_err(&fpc1020->spi->dev, "wait for cmd%d irq timeout\n", cmd);
	}
