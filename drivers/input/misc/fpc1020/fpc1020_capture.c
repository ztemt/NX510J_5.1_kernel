...
int fpc1020_capture_task(fpc1020_data_t *fpc1020)
{
	...
	if (wait_finger_down && fpc1020->capture.deferred_finger_up) {
		...
		dev_info(&fpc1020->spi->dev, "Deferred finger up\n");
	...
	if (wait_finger_up) {
		...
		dev_info(&fpc1020->spi->dev, "Waiting finger up...\n");
		error = fpc1020_capture_wait_finger_up(fpc1020);

		if(error == -EINTR) {
			dev_err(&fpc1020->spi->dev, "Finger up check interrupted\n");
		...
		dev_info(&fpc1020->spi->dev, "Finger up\n");
	}
